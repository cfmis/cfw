...
namespace cfoa
{
    public partial class ShowClrCode : BasePage//System.Web.UI.Page
    {
        ...
        {
            string strSql = "Select id,name,english_name,do_color from cd_color Where id>='' ";
            if (txtClr.Text != "")
                strSql += " AND id Like " + "'%" + txtClr.Text + "%'";
            if (txtName.Text != "")
            ...
            strSql += " Order By id";
            DataTable tbClr = sh.ExecuteSqlReturnDataTableGeo(strSql);
            if (tbClr.Rows.Count == 0)
                tbClr.Rows.Add();

            gvDetails.DataSource = tbClr.DefaultView;
            gvDetails.DataBind();
        }
