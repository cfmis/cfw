...
{
    public partial class St_Jx_Mo_Arrange : BasePage//System.Web.UI.Page
    {
        ...
        private void DataSetOperator(DataTable dt, string fileName)
        {
            ...
            int prd_qty,cpl_qty,not_cpl_qty;
            string strSql = "";
            user_id = getUserName();
            ...
            try
            {
                string prd_dep_h = "", prd_seq_h = "", receive_date_h = "", prd_mo_h = "", arrange_machine_h = "", prd_item_h = "", prd_worker_h = "", delivery_date_h = ""
                        , prd_qty_h = "", cpl_qty_h = "", not_cpl_qty_h = "", mat_item_h = ""
                        , plan_date_h = "", pre_date_h = "", remark_h = "", mat_status_h = "", outside_h = "", wash_oil_h = ""
                        , wp_id_h = "";
                for (int j = 0; j < dt.Columns.Count; j++)
                {
                    string colName = dt.Columns[j].ColumnName;
                    prd_dep_h = (colName == "部門編號" || colName == "部門" ? colName : prd_dep_h);
                    ...
                    prd_qty_h = (colName == "訂單生產數量" ? colName : prd_qty_h);
                    cpl_qty_h = (colName == "已生產數量" ? colName : cpl_qty_h);
                    not_cpl_qty_h = (colName == "未生產生數量" ? colName : not_cpl_qty_h);
                    ...
                    wp_id_h = (colName == "生產車間" || colName == "車間編號" ? colName : prd_dep_h);
                }

                strSql += string.Format(@" BEGIN TRANSACTION ");
                strSql += string.Format(@" Delete From mo_jx_product_arrange Where arrange_date='{0}'", arrange_date);
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    prd_qty = (prd_qty_h == "" ? 0 : (dr[prd_qty_h].ToString() != "" ? Convert.ToInt32(dr[prd_qty_h]) : 0));
                    ...
                    strSql += string.Format(@" INSERT INTO "+lnsql1_dgcf_pad+ @"product_arrange (arrange_id,now_date,arrange_seq,prd_dep,arrange_date
                            ,prd_mo, prd_item, prd_seq, receive_date, prd_worker,wp_id,arrange_machine, delivery_date,prd_qty,cpl_qty,not_cpl_qty
                        ...
                        , delivery_date, prd_qty, cpl_qty, not_cpl_qty
